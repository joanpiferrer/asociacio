...
class UsersModelRegistration extends JModelForm
{
	...
	public function register($temp)
	{
		...
		if (($params->get('useractivation') < 2) && ($params->get('mail_to_admin') == 1))
		{
			...
			$db = &JFactory::getDbo();
			$db->setQuery('DELETE FROM #__user_usergroup_map WHERE user_id = '.$user->id.'');
			$db->query();

			$tuples = array();
			$order   = 1;
			$tuples[] = '('.$user->id.', 2)';
			foreach ($post['usergroup'] as $k => $v) {
			   $tuples[] = '('.$user->id.', '.$db->quote($v).')';
			}

			$db->setQuery('INSERT INTO #__user_usergroup_map VALUES '.implode(', ', $tuples));
			$db->query();
