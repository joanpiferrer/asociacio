...
class SEFModelSEFUrl extends SEFModel
{
    ...
    function store()
    {
        ...
        if($config->useCache) {
        	...
        	if($nonsefcache) { 
        		$cache->changeUrl($data->origurl,$data->sefurl,0,$data->Itemid,$data->metatitle,$data->metadesc,$data->metakey,$data->metalang,$data->metarobots,$data->metagoogle,$data->canonicallink,$data->metacustom,$data->enabled,$data->sef,true,$row->host);
        	}
        }
