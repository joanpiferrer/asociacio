...
class EasyJoomlaBackupModelCreatebackup extends JModelLegacy
{
     ...
     * @return bool
     */
    function createBackup($type, $plugin = false)
    {
        ...
        if(class_exists('ZipArchive'))
        {
            ...
            else
            {
                ...
                $data['date'] = $this->_backup_datetime->toSql();
                $data['type'] = $type;
                $data['name'] = $file_name;
     ...
     * @return boolean
     */
    function createBackupZipArchiveDatabase($file_name)
    {
     ...
     * @return bool
     */
    function zipFoldersAndFilesRecursive($zip, $folder, $folder_relative, $exclude_files = array(), $exclude_folders = array(), $folder_start = false)
    {
    ...
    function backupDatabase($file_name_dump)
    {
        ...
        $data .= '-- Project page: http://joomla-extensions.kubik-rubik.de/ejb-easy-joomla-backup'."\n\n";

        file_put_contents($this->_backup_folder.$file_name_dump, $data);
    ...
    {
        $max_number_backups = $this->_params->get('max_number_backups', 5);
